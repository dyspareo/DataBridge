(function () {
  ...
  async function loadData() {
    ...
    const json = await fetchJson(`${API_BASE}/api/fad/master/names?plantCode=${encodeURIComponent(plantCode)}&deptCode=${encodeURIComponent(deptCode)}`);
  ...
  async function insert(e) {
    ...
    setMsg('Inserting FAD hierarchy...');
    const result = await fetchJson(`${API_BASE}/api/fad/hierarchy`, {
      method: 'POST',
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    $('fadForm')?.addEventListener('submit', (e) => insert(e).catch(err => setMsg(err.message || 'Insert failed', 'error')));
